...
export function CustomerAuthDialog({ onClose, onAuthenticated }: CustomerAuthDialogProps) {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      setError(err instanceof ApiError ? err.message : '登录失败，请检查信息');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#0b1f1a]/55 px-4 py-6 backdrop-blur-sm">
      <form className="relative w-full max-w-md rounded-[28px] border border-white/30 bg-background p-6 shadow-[0_36px_120px_rgba(0,0,0,0.25)]" onSubmit={submit}>
        <button aria-label="关闭登录窗口" className="absolute right-4 top-4 inline-flex size-10 items-center justify-center rounded-full bg-foreground/5 text-foreground/60 hover:bg-foreground/10" onClick={onClose} type="button">
          <X className="size-4" />
        </button>
        ...
        <h2 className="mt-3 font-display text-step-4 font-normal text-foreground">
          {mode === 'login' ? '用户登录' : '注册清濯账号'}
        </h2>
        <p className="mt-3 font-body text-step--1 leading-7 text-muted-foreground">
          登录后下单会自动归属到你的账号，可在“我的订单”查看付款和发货状态。
        </p>

        <div className="mt-6 grid grid-cols-2 gap-2 rounded-full bg-foreground/[0.05] p-1">
          {(['login', 'register'] as const).map((item) => (
            <button
              className={`rounded-full px-4 py-2 font-body text-step--1 transition ${mode === item ? 'bg-background text-foreground shadow-sm' : 'text-muted-foreground'}`}
              key={item}
        ...
        <Button className="mt-6 h-12 w-full rounded-full" disabled={submitting} type="submit" variant="hero">
          {submitting ? '处理中...' : mode === 'login' ? '登录账号' : '创建账号'}
        </Button>
      </form>
    </div>
  );
